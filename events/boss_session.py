...
import os
import sys
from events.boss_list import create_boss
...
class DungeonSession:
    ...
    async def end_dungeon(self, reason: str):
        print(f"🔍 DEBUG: Attempting to end dungeon")
        print(f"🔍 DEBUG: Channel ID: {self.channel_id}")
        print(f"🔍 DEBUG: Channel ID type: {type(self.channel_id)}")

        channel = self.client.get_channel(self.channel_id)  # No need to convert to int
        print(f"🔍 DEBUG: get_channel result: {channel}")

        if channel is None:
            print(f"🔍 DEBUG: Channel not in cache, attempting fetch...")
            try:
                channel = await self.client.fetch_channel(self.channel_id)
                print(f"✅ SUCCESS: Fetched channel: {channel}")
                print(f"🔍 DEBUG: Channel name: {channel.name}")
                print(f"🔍 DEBUG: Channel guild: {channel.guild}")
            except discord.NotFound:
                print("❌ ERROR: Channel not found (404) - Channel was deleted!")
                return
            except discord.Forbidden:
                print("❌ ERROR: Forbidden (403) - No permission to access channel!")
                return
            except Exception as e:
                print(f"❌ ERROR: Unexpected error: {type(e).__name__}: {e}")
                print(f"🔍 DEBUG: Client is ready: {self.client.is_ready()}")
                print(f"🔍 DEBUG: Client user: {self.client.user}")
                return
...
class DungeonJoinView(discord.ui.View):
    ...
    @discord.ui.button(label="Join Raid", style=discord.ButtonStyle.green)
    async def join(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title="🛡️ Boss Raid Recruitment",
            description=f"Party Leader: <@{self.session.leader_id}>\n\n" +
                        "\n".join([f"🧍 <@{uid}>" for uid in self.session.members]),
            color=discord.Color.gold()
        )
        await interaction.message.edit(embed=embed, view=self)
        await interaction.response.send_message(f"{user.mention} joined the raid party!", ephemeral=True)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.session.leader_id:
            await interaction.response.send_message("Only the party leader can cancel this raid.", ephemeral=True)
            return
    ...
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Spawn enemies
        if self.session.is_boss_fight:
            self.session.enemies = [create_boss()]
        else:
